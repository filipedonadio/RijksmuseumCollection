...
final class ItemViewModel {
    ...
    private func downloadImage(imageUrl: String, completion: @escaping (Result<Data, NetworkError>) -> Void) {
        if let url = URL(string: imageUrl.replacingOccurrences(of: "=s0", with: "=w500")) {
    ...
    private func prepareItem(item: Item) {

        // Download item image
        if let webImage = item.webImage {
            downloadImage(imageUrl: webImage.url) { [weak self] response in
                switch response {

                case .failure(let error):
                    print("Error loading the image from URL: \(webImage.url) | Error: \(error)")
                case .success(let imageData):
                    self?.formatData(item: item, image: imageData)
                }
            }
        } else {
            formatData(item: item, image: nil)
        }
    }

    private func formatData(item: Item, image: Data?) {
        ...
        DispatchQueue.main.async { [weak self] in
            self?.delegate?.onFetchCompleted(with: ItemDisplayable(headerImage: image, sections: sections))
        }
    }
    ...
    func fetchItem(objectNumber: String) {
        ...
        itemService.fetch(objectNumber: objectNumber) { [weak self] response in
            ...
            case .success(let itemData):
                self?.prepareItem(item: itemData.artObject)
            }
        }
    }
}
